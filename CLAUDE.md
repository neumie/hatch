...
- **Running Docker is not the same as existing Docker state.** `hatch setup` must distinguish running services from stopped containers/persistent volumes. Seed imports are only valid for a fresh workspace; importing into a stopped but already-migrated Contember project produces an incompatible-schema failure. `hatch reset` is a hard-reset command: teardown must verify that Compose containers and volumes are gone before setup starts, and import failure must abort before tail migrations/hooks run.
