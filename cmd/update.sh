...
# On reload after self-update, skip git pull (already done) and run post-update steps
if [[ "${_HATCH_UPDATE_RELOADED:-}" != "1" ]]; then
...
if [[ -d "$HATCH_HOME/.git" ]]; then
  ...
  # Hash this script before pull to detect self-changes
  _update_hash_before=$(_md5 "$(cat "$HATCH_HOME/cmd/update.sh")")
  ...
  # If this script changed during pull, re-source the new version
  # so any new post-update steps execute immediately
  _update_hash_after=$(_md5 "$(cat "$HATCH_HOME/cmd/update.sh")")
  if [[ "$_update_hash_before" != "$_update_hash_after" ]]; then
    _HATCH_UPDATE_RELOADED=1 source "$HATCH_HOME/cmd/update.sh"
    unset _HATCH_UPDATE_RELOADED _update_hash_before _update_hash_after
