...
# Writes .env file with WORKSPACE_NAME, PROJECT_NAME, and all PORT_* variables
# Used for Docker Compose variable interpolation
hatch_write_env() {
  local workspace_name="$1"
  local project_name="${2:-$workspace_name}"
  local env_file=".env"

  _info "Writing Docker Compose .env file"

  # Remove existing symlink to avoid writing through it into secrets
  if [[ -L "$env_file" ]]; then
    rm -f "$env_file"
  fi

  {
    ...
    echo "COMPOSE_PROJECT_NAME=${project_name}-${workspace_name}"
...
hatch_write_docker_override() {
  ...
  # Start YAML file
  {
    ...
    while IFS= read -r service_spec; do
      ...
      echo "  ${service_name}:"
      echo "    container_name: \${PROJECT_NAME}-\${WORKSPACE_NAME}-${service_name}"
      echo "    ports: !override"
