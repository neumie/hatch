...
**Data export (local)** — CLI: `contember data:export --output <path>`. Runs via the Docker-wrapped CLI (e.g., `_pkg_run contember data:export`). Output path must be within the Docker volume mount.

**Data export (remote)** — CLI: `contember data:export contember://project:token@host --output <path>`. The source URI uses `contember://` protocol with project slug and bearer token embedded. Used by `hatch export --remote` to pull data directly from production/staging without a local instance. Hatch provides `HATCH_REMOTE_URL` (e.g., `contember://jvs@api-jvs.eu.contember.cloud`) and `HATCH_REMOTE_TOKEN` as env vars; the hook injects the token into the URI.
...
import { importData, exportData, exportRemoteData } from "./lib/data.js";
import {
  getApiUrl,
  getDockerHost,
  createContentClient,
...
export async function project_remote_export(exportPath: string) {
  await exportRemoteData(
    exportPath,
    process.env.HATCH_REMOTE_URL!,
    process.env.HATCH_REMOTE_TOKEN!,
  );
}
